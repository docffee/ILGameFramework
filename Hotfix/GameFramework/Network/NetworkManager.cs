...
namespace GameFramework.Network
{
    ...
    internal sealed partial class NetworkManager : GameFrameworkModule, INetworkManager
    {
        ...
        public INetworkChannel CreateNetworkChannel(string name, INetworkChannelHelper networkChannelHelper)
        {
            ...
            {
                throw new System.Exception("Network channel helper is invalid.");
            }

            if (networkChannelHelper.PacketHeaderLength <= 0)
            {
                throw new System.Exception("Packet header length is invalid.");
            }

            if (HasNetworkChannel(name))
            {
                throw new System.Exception(string.Format("Already exist network channel '{0}'.", name ?? string.Empty));
            }
