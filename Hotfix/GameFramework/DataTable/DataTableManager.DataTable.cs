        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            public void AddDataRow(string dataRowText)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new System.Exception(string.Format("Can not parse data table '{0}' at '{1}' with exception '{2}'.", Utility.Text.GetFullName<T>(Name), dataRowText, exception.ToString()), exception);
                }

                if (HasDataRow(dataRow.Id))
                {
                    throw new System.Exception(string.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id.ToString(), Utility.Text.GetFullName<T>(Name)));
                }
