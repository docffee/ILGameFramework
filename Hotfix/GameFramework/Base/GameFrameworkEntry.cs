...
namespace GameFramework
{
    ...
    {
        private const string GameFrameworkVersion = "3.0.6";
        ...
        /// 获取游戏框架模块。
        /// </summary>  
        /// <typeparam name="T">要获取的游戏框架模块类型。不再需要之前的接口类型，热更新对接口支持不好</typeparam>
        ...
        public static T GetModule<T>() where T : class
        {
            ...
            //热更新不识别interface   暂时注释掉--换个诡异的判断方法
            //if (!interfaceType.IsInterface)
            //{
            //    throw new System.Exception(string.Format("You must get module by interface, but '{0}' is not.", interfaceType.FullName));
            //}
            ...
            //ilrt对接口支持的真不怎样 不识别命名空间 再换一种方法
            //string moduleName = string.Format("{0}.{1}", interfaceType.Namespace, interfaceType.Name.Substring(1));
            string moduleName = interfaceType.FullName.Replace(interfaceType.Name, interfaceType.Name.Substring(1));

           // 把原来接口类型 转管理类型注释 直接使用管理类
