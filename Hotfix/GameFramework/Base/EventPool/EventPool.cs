...
namespace GameFramework
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new System.Exception(string.Format("Event '{0}' not allow multi handler.", id.ToString()));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) == 0 && Check(id, handler))
            {
                throw new System.Exception(string.Format("Event '{0}' not allow duplicate handler.", id.ToString()));
            }
            else
            {
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new System.Exception(string.Format("Event '{0}' not allow no handler.", e.Id.ToString()));
            }
        }
    }
}
